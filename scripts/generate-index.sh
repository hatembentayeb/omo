...
#   - dist/plugins/*-{os}-{arch}.tar.gz (RPC plugin executables)
...
# Artifacts are RPC plugin executables (not Go native .so).
...
for name in $PLUGIN_NAMES; do
  ...
  AMD64_FILE="$DIST_DIR/${name}-${VERSION}-linux-amd64.tar.gz"
  ARM64_FILE="$DIST_DIR/${name}-${VERSION}-linux-arm64.tar.gz"
