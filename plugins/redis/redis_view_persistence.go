...
func (rv *RedisView) newPersistenceView() *ui.CoreView {
	cores := ui.NewCoreView(rv.app, "Redis Persistence")
	cores.SetTableHeaders([]string{"Property", "Value"})
	cores.SetRefreshCallback(rv.refreshPersistence)
...
func (rv *RedisView) refreshPersistence() ([][]string, error) {
	if rv.redisClient == nil || !rv.redisClient.IsConnected() {
		return [][]string{{"Status", "Not Connected"}}, nil
	}

	infoMap, err := rv.redisClient.GetInfoSectionMap("persistence")
	if err != nil {
		return [][]string{}, err
	}
